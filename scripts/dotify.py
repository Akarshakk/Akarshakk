...
In --color mode both files carry the photo's own colours; the light one is
shaded down (--color-shade) so a lit subject doesn't vanish against white.
...
# In --color mode the fills come from the photo, so on a white page a brightly
# lit subject renders as big near-white dots and vanishes. Darkening the photo's
# own colours for the light theme keeps the hue but puts the ink back.
COLOR_SHADE = {"dark": 1.0, "light": 0.58}
...
def shade_hex(cr, cg, cb, k):
    """Scale a photo colour toward black by k, keeping its hue."""
    if k >= 1.0:
        return f"#{cr:02x}{cg:02x}{cb:02x}"
    return "#%02x%02x%02x" % (int(cr * k), int(cg * k), int(cb * k))


def build_dots(cols, rows, lum, rgb, theme, opts):
    fg, dim, _ = THEMES[theme]
    shade = opts.color_shade if theme == "light" else COLOR_SHADE["dark"]
    cell = opts.cell
...
def main(argv=None):
    ...
    p.add_argument("--color-shade", type=float, default=COLOR_SHADE["light"],
                   metavar="K",
                   help="with --color, how far to darken photo colours for the "
                        "LIGHT theme so they stay visible on white "
                        f"(default {COLOR_SHADE['light']}; 1.0 disables it)")
    ...
    # Both themes are always emitted. In --color mode the fills come from the
    # photo, but the light render shades them down (see COLOR_SHADE) so the
    # subject doesn't disappear into a white page.
    themes = ("dark", "light")
    for theme in themes:
        body, w, h = builder(cols, rows, lum, rgb, theme, args)
        svg = svg_header(w, h, rows, args) + body + "</g></svg>"
        stem = f"{args.out.name}-{theme}"
